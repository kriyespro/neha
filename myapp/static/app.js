...
async function syncDatabaseState() {
  ...
  // 3. Coupons — no longer pre-fetched; validated on-demand via /api/coupons/validate
...
async function handleCouponApply() {
  ...
  try {
    ...
    if (res.ok && data.valid) {
      activeAppliedCoupon = { code: input, discountPercent: data.discountPercent, isActive: true };
      feedback.style.color = "green";
      feedback.textContent = `Coupon Applied: ${data.discountPercent}% OFF!`;
      updateCartTotals();
