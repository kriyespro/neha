...
function setDynamicImagePaths() {
  document.documentElement.style.setProperty(
    "--hero-bg-image",
    `url("${resolveImageUrl("hero-new1.jpg")}")`
  );
  document.documentElement.style.setProperty(
    "--promo-bg-image",
...
async function vgInit() {
  ...
  // Coupons — validated on-demand via /api/coupons/validate (not pre-fetched)
...
async function applyCoupon() {
  ...
  try {
    ...
    if (res.ok && data.valid) {
      const found = { code, discountPercent: data.discountPercent, isActive: true };
      VG.appliedCoupon = found;
      localStorage.setItem("vg_applied_coupon", JSON.stringify(found));
      const msg = `✓ Coupon "${code}" applied — ${found.discountPercent}% off!`;
      if (fb) { fb.textContent = msg; fb.style.color = "var(--color-success, #4caf7d)"; }
      if (pageFb) { pageFb.textContent = msg; pageFb.style.color = "var(--color-success, #4caf7d)"; }
    } else {
      VG.appliedCoupon = null;
      localStorage.removeItem("vg_applied_coupon");
      const msg = data.error || "Invalid or expired coupon code.";
      if (fb) { fb.textContent = msg; fb.style.color = "#e05c5c"; }
      if (pageFb) { pageFb.textContent = msg; pageFb.style.color = "#e05c5c"; }
    }
  } catch (e) {
    VG.appliedCoupon = null;
    localStorage.removeItem("vg_applied_coupon");
    const msg = "Could not validate coupon. Please try again.";
    if (fb) { fb.textContent = msg; fb.style.color = "#e05c5c"; }
    if (pageFb) { pageFb.textContent = msg; pageFb.style.color = "#e05c5c"; }
  }
  updateCartTotals();
